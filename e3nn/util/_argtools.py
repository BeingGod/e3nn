...
def _transform(dat, irreps_dat, rot_mat, translation=0.0, output_transform_dtype: bool = False):
    ...
    for irreps, a in zip(irreps_dat, dat):
        ...
        if irreps is None:
            ...
            out.append((a.astype(transform_dtype) @ rot_mat.T.astype(a.dtype) + translation).astype(out_dtype))
        else:
            out.append((a.astype(transform_dtype) @ irreps.D_from_matrix(rot_mat).T.astype(a.dtype)).astype(out_dtype))
    return out
...
def _get_io_irreps(func, irreps_in=None, irreps_out=None):
    ...
    if (irreps_in is None or irreps_out is None) and isinstance(func, paddle.nn.Layer):
        warnings.warn("Asking to infer irreps in/out of a module. Please provide `irreps_in` " "and `irreps_out` explicitly.")
...
def _rand_args(irreps_in, batch_size: Optional[int] = None):
    ...
    args_in = [
        (paddle.randn([batch_size, 3]) if (irreps == "cartesian_points") else irreps.randn(batch_size, -1))
...
def _to_device_dtype(args, device=None, dtype=None):
    ...
    if isinstance(args, paddle.Tensor):
        if args.dtype in ["float32", "float64"]:
            return args.astype(dtype=dtype).cuda() if device == "gpu" else args.astype(dtype=dtype)
        else:
