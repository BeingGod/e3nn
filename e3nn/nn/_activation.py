...
class Activation(paddle.nn.Layer):
    ...
    >>> a = Activation("256x0o", [paddle.abs])
    >>> a.irreps_out
    256x0e
    ...
    def forward(self, features, dim=-1):
        ...
        for (mul, ir), act in zip(self.irreps_in, self.acts):
            if act is not None:
                ...
                output.append(act(paddle.slice(features, [dim], [start_0], [start_0 + mul])))
            else:
                start_1 = features.shape[dim] + index if index < 0 else index
                output.append(paddle.slice(features, [dim], [start_1], [start_1 + mul * ir.dim]))
            index += mul * ir.dim
        if len(output) > 1:
