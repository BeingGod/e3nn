...
def orthonormalize(original: paddle.Tensor, eps: float = 1e-09) -> Tuple[paddle.Tensor, paddle.Tensor]:
    """orthonomalize vectors

    Parameters
    ----------
    original : `paddle.Tensor`
    ...
    for i, x in enumerate(original):
        ...
        if x.norm() > 2 * eps:
            ...
            final += [x]
            matrix += [cx]
    final = paddle.stack(x=final) if len(final) > 0 else paddle.zeros(shape=(0, dim), dtype=original.dtype)
    matrix = paddle.stack(x=matrix) if len(matrix) > 0 else paddle.zeros(shape=(0, len(original)), dtype=original.dtype)
    return final, matrix
...
def complete_basis(vecs: paddle.Tensor, eps: float = 1e-09) -> paddle.Tensor:
    ...
    for x in paddle.eye(num_rows=dim, dtype=vecs.dtype):
        ...
        if x.norm() > 2 * eps:
            ...
            expand += [x]
    expand = paddle.stack(x=expand) if len(expand) > 0 else paddle.zeros(shape=[0, dim], dtype=vecs.dtype)
    return expand
