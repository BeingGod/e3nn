...
def flat_wigner(lmax, alpha, beta, gamma):
    return paddle.concat(
        x=[((2 * l + 1) ** 0.5 * wigner_D(l, alpha, beta, gamma).flatten(-2)) for l in range(lmax + 1)],
        axis=-1,
    )
...
class SO3Grid(paddle.nn.Layer):
    ...
    def __init__(self, lmax, resolution, *, normalization="component", aspect_ratio=2):
        ...
        self.register_buffer(
            name="D",
            tensor=flat_wigner(lmax, a[:, None, None], b[None, :, None], a[None, None, :]),
        )
    ...
    def to_grid(self, features):
        ...
        """
        return paddle.einsum("...i,abci->...abc", features, self.D) / tuple(self.D.shape)[-1] ** 0.5
    ...
    def from_grid(self, features):
        ...
        `paddle.Tensor`
            tensor of shape ``(..., self.irreps.dim)``
        """
        return paddle.einsum("...abc,abci,b->...i", features, self.D, self.qw) * tuple(self.D.shape)[-1] ** 0.5
