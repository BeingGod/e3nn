        ...
        if self.normalization == "norm":
            out.divide_(
                y=paddle.to_tensor(
                    paddle.concat(
                        x=[
                            (math.sqrt(2 * l + 1) / math.sqrt(4 * math.pi) * paddle.ones(shape=2 * l + 1, dtype=out.dtype))
            ...
            out.multiply_(y=paddle.to_tensor(math.sqrt(4 * math.pi), dtype=out.dtype))
...
def _mul_m_lm(mul_l: List[Tuple[int, int]], x_m: paddle.Tensor, x_lm: paddle.Tensor) -> paddle.Tensor:
    """
