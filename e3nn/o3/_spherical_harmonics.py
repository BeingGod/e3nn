...
class SphericalHarmonics(paddle.nn.Layer):
    ...
    def forward(self, x: paddle.Tensor) -> paddle.Tensor:
        ...
        if not self._is_range_lmax:
            sh = paddle.concat(x=[sh[..., l * l : (l + 1) * (l + 1)] for l in self._ls_list], axis=-1)
        if self.normalization == "integral":
            sh = sh / paddle.to_tensor(math.sqrt(4 * math.pi))
        elif self.normalization == "norm":
            sh = sh / paddle.concat(
                x=[(math.sqrt(2 * l + 1) * paddle.ones(shape=2 * l + 1, dtype=sh.dtype)) for l in self._ls_list]
            )
...
def spherical_harmonics(
    ...
    * are orthogonal :math:`\\int_{S^2} Y^l_m(x) Y^j_n(x) dx = \\text{cste} \\; \\delta_{lj} \\delta_{mn}`
    ...
    >>> spherical_harmonics(0, paddle.randn([2, 3]), False, normalization='component')
    tensor([[1.],
            [1.]])
...
def _spherical_harmonics(lmax: int, x: paddle.Tensor, y: paddle.Tensor, z: paddle.Tensor) -> paddle.Tensor:
    ...
    sh_11_1 = (
        1 / 11 * math.sqrt(23) * sh_10_0 * y + 1 / 11 * math.sqrt(115) * sh_10_1 * z + 1 / 11 * math.sqrt(115) * sh_10_19 * x
    )
    sh_11_2 = (
    ...
    sh_11_21 = (
        -1 / 11 * math.sqrt(115) * sh_10_1 * x + 1 / 11 * math.sqrt(115) * sh_10_19 * z + 1 / 11 * math.sqrt(23) * sh_10_20 * y
    )
...
def _generate_spherical_harmonics(lmax, device=None):
    ...
    print("sh_0_0 = paddle.ones_like(x)")
    print("if lmax == 0:")
    print("    return paddle.stack([")
    print("        sh_0_0,")
    print("    ], axis=-1)")
    print()
