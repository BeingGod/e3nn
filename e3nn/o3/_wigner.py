...
def change_basis_real_to_complex(l: int, dtype=None, device=None) -> paddle.Tensor:
    ...
    q = paddle.full((), fill_value=-1.0j, dtype=q.dtype) ** l * q
...
def wigner_D(l, alpha, beta, gamma):
    ...
    * :math:`D(\\text{rotation around Y axis})` has some property that allows us to use FFT in `ToS2Grid`
    ...
    X = so3_generators(l)
    return (
        paddle.linalg.matrix_exp(alpha * X[1]) @ paddle.linalg.matrix_exp(beta * X[0]) @ paddle.linalg.matrix_exp(gamma * X[1])
    )
...
def wigner_3j(l1, l2, l3, dtype=None, device=None):
    ...
    dtype : paddle.dtype or None
        ``dtype`` of the returned tensor. If ``None`` then set to ``paddle.get_default_dtype()``.
...
def _su2_clebsch_gordan(j1, j2, j3):
    ...
    mat = paddle.zeros(shape=(int(2 * j1 + 1), int(2 * j2 + 1), int(2 * j3 + 1)), dtype="float64")
