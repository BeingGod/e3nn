    ...
    w = paddle.to_tensor(
        data=[
            (
                ...
                * (1.0 / (2 * k + 1) * paddle.sin(x=(2 * j + 1) * (2 * k + 1) * math.pi / (4.0 * b))).sum()
            )
...
def s2_grid(res_beta, res_alpha, dtype=None, device=None):
    ...
    dtype : paddle.dtype or None
        ``dtype`` of the returned tensors. If ``None`` then set to ``paddle.get_default_dtype()``.

    device : paddle.base.libpaddle.Place or None
...
def _expand_matrix(ls, like=None, dtype=None, device=None):
    ...
    lmax = max(ls)
    if like is None:
        m = paddle.zeros(shape=[len(ls), 2 * lmax + 1, sum(2 * l + 1 for l in ls)], dtype=dtype)
    else:
        m = paddle.zeros(shape=(len(ls), 2 * lmax + 1, sum(2 * l + 1 for l in ls)), dtype=dtype)
    i = 0
    for j, l in enumerate(ls):
        m[j, lmax - l : lmax + l + 1, i : i + 2 * l + 1] = paddle.eye(num_rows=2 * l + 1, dtype=dtype)
        i += 2 * l + 1
    return m
...
def irfft(x, res):
    ...
    *size, sm = tuple(x.shape)
    ...
    l = res // 2
    x = paddle.complex(
        real=paddle.concat(x=[x[:, l : l + 1], x[:, l + 1 :].div(math.sqrt(2))], axis=1),
        imag=paddle.concat(
            x=[
...
class ToS2Grid(paddle.nn.Layer):
    ...
    dtype : paddle.dtype or None, optional
    device : paddle.base.libpaddle.Place or None, optional
    ...
    def __init__(self, lmax=None, res=None, normalization="component", dtype=None, device=None):
        ...
        if normalization == "norm":
            n = math.sqrt(4 * math.pi) * paddle.ones(shape=lmax + 1, dtype=betas.dtype) / math.sqrt(lmax + 1)
...
class FromS2Grid(paddle.nn.Layer):
    ...
    def __init__(
        ...
        if normalization == "norm":
            n = math.sqrt(4 * math.pi) * paddle.ones(shape=lmax + 1, dtype=betas.dtype) * math.sqrt(lmax_in + 1)
        if normalization == "integral":
        ...
        m = _expand_matrix(range(lmax + 1), dtype=dtype, device=device)
        assert res_beta % 2 == 0
        qw = _quadrature_weights(res_beta // 2, dtype=dtype, device=device) * res_beta**2 / res_alpha
    ...
    def _make_tracing_inputs(self, n: int):
        return [{"forward": (paddle.randn(shape=[self.res_beta, self.res_alpha]),)} for _ in range(n)]
