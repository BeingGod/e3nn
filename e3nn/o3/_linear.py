...
class Linear(CodeGenMixin, paddle.nn.Layer):
        ...
        *,
        ...
        if internal_weights and self.weight_numel > 0:
            ...
            self.weight = paddle.base.framework.EagerParamBase.from_tensor(
                tensor=paddle.randn(
                    shape=[
                        *((f_in, f_out) if f_in is not None else ()),
                        self.weight_numel,
                    ]
                )
            )
        ...
        if internal_weights and self.bias_numel > 0:
            ...
            self.bias = paddle.base.framework.EagerParamBase.from_tensor(
                tensor=paddle.zeros(shape=[*((f_out,) if f_out is not None else ()), self.bias_numel])
            )
        else:
            self.register_buffer(name="bias", tensor=paddle.to_tensor(data=[]))
        if self.irreps_out.dim > 0:
            output_mask = paddle.concat(
                x=[
                    (
                        paddle.ones(shape=mul_ir.dim)
                        if any(ins.i_out == i_out and 0 not in ins.path_shape for ins in self.instructions)
                        else paddle.zeros(shape=mul_ir.dim)
                    )
    ...
    def weight_view_for_instruction(self, instruction: int, weight: Optional[paddle.Tensor] = None) -> paddle.Tensor:
        ...
        if weight is None:
            ...
            weight = self.weight
        batchshape = tuple(weight.shape)[:-1]
        ...
        return paddle.slice(weight, [-1], [start_0], [start_0 + prod(ins.path_shape)]).view(batchshape + ins.path_shape)
    ...
    def weight_views(self, weight: Optional[paddle.Tensor] = None, yield_instruction: bool = False):
        ...
        for ins_i, ins in enumerate(self.instructions):
            ...
            this_weight = paddle.slice(weight, [-1], [start_1], [start_1 + flatsize]).view(batchshape + ins.path_shape)
            offset += flatsize
...
def _codegen_linear(
    ...
    def forward_fn(x: paddle.Tensor, ws: paddle.Tensor, bs: paddle.Tensor) -> paddle.Tensor:
        if f_in is None:
            size = x.shape[:-1]
        ...
        if len(irreps_in) == 1:
            ...
            for mul_ir in irreps_in:
                ...
                x_list.append(x_slice.reshape([batch_out] + ([] if f_in is None else [f_in]) + [mul_ir.mul, mul_ir.ir.dim]))
                start += mul_ir.dim
        ...
        for ins in instructions:
            ...
            if ins.i_in == -1:
                ...
                w = w.reshape(
                    ([] if shared_weights else [-1]) + ([] if f_in is None else [f_in, f_out]) + list(ins.path_shape)
                )
                flat_weight_index += path_nweight
